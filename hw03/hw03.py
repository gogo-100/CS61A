...
def pingpong_score(start,now,end,k):
    if now >= end:
        return k* (now - start)

    if now % 8 == 0 or num_eights(now) != 0:
        return k* (now - start) + pingpong_score(now, now + 1, end, -k)
    else:
        return pingpong_score(start,now+1,end,k)
...
def count_partition(change,max_coin):
    if change < 0:
        return 0
    if change == 0: # Just finished partition,solution +1
        return 1
    if max_coin is None:
        return 0

    # Divide into two sub-cases: no max_coin at all and max_coin at least once
    return count_partition(change,next_smaller_coin(max_coin)) + count_partition(change-max_coin,max_coin)  
