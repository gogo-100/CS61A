...
def berry_finder(t):
    ...
    True
    """
    return label(t) == 'berry' or any([berry_finder(subtree) for subtree in branches(t)])
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        return tree(label(t),[sprout_leaves(subtree,leaves) for subtree in branches(t)])
...
def add_trees(t1, t2):
    ...
    """
    res = []
    for pair in zip(branches(t1),branches(t2)):
        res.append(add_trees(pair[0],pair[1]))
    if len(branches(t1)) != len(branches(t2)):
      size = len(res)
      longer = t1 if len(branches(t1)) > len(branches(t2)) else t2
      res += [subtree for i,subtree in enumerate(branches(longer)) if i >= size]
