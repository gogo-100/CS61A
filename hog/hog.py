...
def play(strategy0, strategy1, update,
    ...
    while True:
        score[who] = update(strategy[who](score[who],score[1-who]),score[who],score[1-who],dice)
        if score[who] >= goal:
            break;
        who = 1 - who
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    maxTime = 0
    for i in range(1,11):
        val = make_averaged(roll_dice,total_samples)(i,dice)
        if val > maxVal:
            maxVal = val
            maxTime = i
    return maxTime
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    test_score = tail_points(opponent_score) + score
    if perfect_square(test_score):  # Implement perfect_square
        test_score = next_perfect_square(test_score)  # Implement next_perfect_square
    return  0 if test_score - score >= threshold else num_rolls 
    # END PROBLEM 11


def final_strategy(score, opponent_score, dice=six_sided, goal=100):
    """Write a brief description of your final strategy.
    Check whether you can win by rolling 0, 1 or 2 dice. If you are in the lead, 
    you might decide to take fewer risks.
    """
    # BEGIN PROBLEM 12
    if score + tail_points(opponent_score) >= goal:
        return 0
    
    for i in range(1, 6):
        if score + make_averaged(roll_dice, 100)(i,dice) >= goal:
            return i
