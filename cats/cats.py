...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    result = list(filter(select,paragraphs))
    if len(result) > k:
        return result[k]
    else:
        return ''
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def contain_topic(str):
        return any(item in remove_punctuation(str).lower().split(' ') for item in topic)
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        return 0.0
    score = sum([1 for x,y in zip(typed_words,source_words) if x == y])
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        lmt = diff_function(typed_word,word,limit)
        if lmt < val or (ans == typed_word and lmt == val):
            ans = word
            val = lmt
    return ans
    # END PROBLEM 5
